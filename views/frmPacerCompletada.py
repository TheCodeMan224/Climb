...
class frmPacerCompletada:
        ...
        if not self.sugerencias:
            self.cont_sugerencias.controls.append(
                ft.Column(spacing=16, controls=[
                    ft.Text(_T["sin_sugerencias"], size=15, italic=True, font_family=tema.FUENTE_SERIF, color=tema.MUTED),
                    ft.Row(spacing=20, controls=[
                        cmp.boton_primario(_T["generar"], on_click=lambda e: self.router.navegar_a("/pacer")),
                        cmp.enlace(_T["volver_dashboard"], on_click=lambda e: self.router.navegar_a("/menu_inicio")),
                    ]),
                ])
            )
            return
        ...
        return ft.Container(
            ...
            content=ft.Row(spacing=22, vertical_alignment=ft.CrossAxisAlignment.START, controls=[
                ...
                ft.Column(expand=True, spacing=8, controls=[
                    ...
                    cmp.eyebrow(_T["n_acciones"].format(n=len(acciones)), color=tema.HINT, size=10),
                ]),
                cmp.enlace_cta(_T["elegir"], on_click=lambda e, mm=mision: self._elegir(mm)),
            ]),
        )
    ...
    def construir(self):
        ...
        intro = ft.Column(horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=0, controls=[
            cmp.eyebrow(_T["eyebrow"], color=tema.AMBAR),
            ft.Container(height=14),
            cmp.hairline(width=48),
            ft.Container(height=18),
            ft.Text(self.mision.get("nombre_mision", _T["titulo_fallback"]), size=40, weight=ft.FontWeight.W_700, font_family=tema.FUENTE_DISPLAY, color=tema.NAVY, text_align=ft.TextAlign.CENTER),
            ft.Container(height=14),
